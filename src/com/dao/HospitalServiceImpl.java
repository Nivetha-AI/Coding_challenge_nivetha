...
public class HospitalServiceImpl implements IHospitalServiceImpl {
	...
	public Appointment getAppointmentById(int appointmentId) throws SQLException {
		...
		if (rst.next()) {
			...
			// save it in object
			Appointment ap = new Appointment(appointmentId1, patientId, doctorId, LocalDate.parse(appointmentDate),
					description);
			return ap;
		}
	...
	public List<Appointment> getAppointmentsForPatient(int patientId)
		...
		while (rst.next()) {
			...
			String description = rst.getString("description");
			// save it in object
			Appointment ap = new Appointment(appointmentId, patientId1, doctorId, LocalDate.parse(appointmentDate),
					description);
			list.add(ap);
		}
		if (list.isEmpty()) {
	...
	public List<Appointment> getAppointmentsForDoctor(int doctorId) throws SQLException {
		...
		while (rst.next()) {
			...
			String description = rst.getString("description");
			// save it in object
			Appointment ap = new Appointment(appointmentId, patientId1, doctorId1, LocalDate.parse(appointmentDate),
					description);
			list.add(ap);
		}
		DBUtilProperty.dbclose();
